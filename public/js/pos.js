...
function pos_each_row(row_obj) {
 ...
 function pos_line_total_row(obj) {
        var total_quantity = 0;
        var price_total = 0;
    
       
        total_quantity = total_quantity + __read_number(obj.find('input.pos_quantity'));
        price_total = price_total + __read_number(obj.find('input.pos_line_total'));
    
    
        //Go through the modifier prices.
        $('input.modifiers_price').each(function() {
            price_total = price_total + __read_number(obj);
        });
       ...
       // $('span.pos_line_real_total').html(__currency_trans_from_en(price_total, false));
        calculate_billing_details1(price_total,obj);
     }
...
function calculate_billing_details1(price_total,obj) {
    var discount = pos_discount(price_total);
    if ($('#reward_point_enabled').length) {
        total_customer_reward = $('#rp_redeemed_amount').val();
        discount = parseFloat(discount) + parseFloat(total_customer_reward);
    ...
    $('input#round_off_amount').val(round_off_amount);
    //__write_number(obj.find('.pos_line_real_total'), total_payable_rounded);
    obj.find('.pos_line_real_total').text(__currency_trans_from_en(total_payable_rounded, true));
