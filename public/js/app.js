$(document).ready(function () {
    ...
    $('#todays_profit_modal').on('shown.bs.modal', function () {
        ...
        var end = start;
        var location_id = '';
        var reporttype = $("input[name='profit_loss_reporttype']:checked").val();
        var displaytype = 2;
        updateProfitLoss(start, end, location_id, $('#todays_profit'), reporttype, displaytype);
...
function updateProfitLoss(start = null, end = null, location_id = null, selector = null, reporttype = null, displaytype = null) {
    if (start == null) {
    ...
    if (displaytype == null) {
        var displaytype = $("#profit_loss_displaytype").val();
    }
    var data = { start_date: start, end_date: end, location_id: location_id, reporttype: reporttype, displaytype: displaytype };
    ...
    $.ajax({
        ...
        success: function (html) {
            ...
            $('#profit_loss_table_wrapper').DataTable({
                dom: 'Bfrtip',
                searching: false,
                paging: false,
                info: false,
...
$(document).on('change', '#todays_profit input[name="profit_loss_reporttype"]', function () {
    var start = $('#modal_today').val();
    var end = start;
    var location_id = '';
    var reporttype = $("input[name='profit_loss_reporttype']:checked").val();
...
$(document).on('click', '.view_purchase_document', function (e) {
    e.preventDefault();
    $('.view_purchase_document_modal').html('');
    $('.view_purchase_document_modal').modal('show');
    $('.view_purchase_document_modal').html('<div class="loader"><div class="spinner-border" role="status"><span class="sr-only">Loading...</span></div></div>');
    $.ajax({
        ...
        success: function (data) {
            $('.view_purchase_document_modal').html(data.documents);
        }
    });
});

$(document).on('change', '#deposit_user_type', function () {
    if (this.value == 'customer') {
        $('.deposit_supplier_dd').hide();
        $('.deposit_customer_dd').show();
    } else if(this.value == 'supplier'){
        $('.deposit_supplier_dd').show();
        $('.deposit_customer_dd').hide();
    }else {
        $('.deposit_customer_dd').hide();
        $('.deposit_supplier_dd').hide();
    }
