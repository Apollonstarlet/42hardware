...
class AccountController extends Controller
{
    ...
    public function show($id)
    {
        ...
        if (request()->ajax()) {
            $accounts = AccountTransaction::join(
                ...
                ->leftJoin('contacts AS cut', 'account_transactions.user_id', '=', 'cut.id')
                ->where('A.business_id', $business_id)
                ->where('A.id', $id)
                ->with(['transaction', 'transaction.contact', 'transfer_transaction', 'media', 'transfer_transaction.media'])
                ->select([
                    'account_transactions.type', 'account_transactions.amount', 'account_transactions.operation_date',
                    'TP.document as tp_document',
                    ...
                    'cut.name as contact_name',
                    'account_transactions.user_type',
                    DB::raw("CONCAT(COALESCE(u.surname, ''),' ',COALESCE(u.first_name, ''),' ',COALESCE(u.last_name,'')) as added_by")
                ])
            ...
            if (!empty(request()->input('user_id'))) {
                $accounts->where('account_transactions.created_by', request()->input('user_id'));
            }
            if (!empty(request()->input('business_id'))) { 
                $accounts->where('transactions.business_id', request()->input('business_id'));
            }
            if (!empty(request()->input('supplier_id'))) { 
                $accounts->where('contacts.id', request()->input('supplier_id'))->orWhere('cut.id', request()->input('supplier_id'));
            }
            if (!empty(request()->input('customer_id'))) { 
                $accounts->where('contacts.id', request()->input('customer_id'))->orWhere('cut.id', request()->input('customer_id'));
            }
            if (!empty(request()->input('reference_no'))) { 
                $accounts->where('transactions.ref_no', 'LIKE', '%' . request()->input('reference_no') . '%');
            }
            ...
            //dd($accounts->get()->toArray());

            return DataTables::of($accounts)
                ->addColumn('debit', function ($row) {
                    if ($row->type == 'debit') {
                        return '<span class="display_currency debit_amount" data-currency_symbol="true"  data-orig-value="' . $row->amount . '">' . $row->amount . '</span>';
                    }
                    return '';
                })
                ->addColumn('credit', function ($row) {
                    if ($row->type == 'credit') {
                        return '<span class="display_currency credit_amount" data-currency_symbol="true"  data-orig-value="' . $row->amount . '">' . $row->amount . '</span>';
                    }
                    return '';
                })
                ->editColumn('balance', function ($row) {
                ...
                ->addColumn('action', function ($row) {
                    ...
                    if (!empty($row->tp_document)) {
                        $action .='<a href="'.asset('/uploads/documents/'.$row->tp_document).'" class="btn btn-success btn-xs" download="001.jpg"><i class="fa fa-download" data-toggle="tooltip" title="" data-original-title="Download Document"></i></a>&nbsp&nbsp';
                        $action .= '<button data-href="'.asset('/uploads/documents/'.$row->tp_document).'" class="btn btn-info btn-xs view_uploaded_document" data-toggle="tooltip" title="View Document"><i class="fa fa-eye"></i></button>';
                    }
                    return $action;
                })
                ->removeColumn('id')
                ->removeColumn('is_closed')
                ->rawColumns(['credit', 'debit', 'balance','added_by', 'sub_type', 'action'])
                ->make(true);
        }
        ...
        return view('account.show')
            ->with(compact('account','users','business_locations','types','suppliers','customers'));
    }
    ...
    public function getDeposit($id)
    {
        ...
        if (request()->ajax()) {
            ...
            return view('account.deposit')
                ->with(compact('account', 'account', 'from_accounts','suppliers','customers'));
        }
    }
    ...
    public function postDeposit(Request $request)
    {
        ...
        try {
            ...
            $deposit_user_id = null;
            if($user_type == 'supplier'){
                $deposit_user_id = $request->input('deposit_supplier_id');
            }
            if($user_type == 'customer'){
                $deposit_user_id = $request->input('deposit_customer_id');
            }
            ...
            if (!empty($amount)) {
                ...
                if (!empty($from_account)) {
                    ...
                    $debit_data['user_type'] = $user_type;
                    $debit_data['user_id'] = $deposit_user_id;
        ...
        if (!empty($row->sub_type)) {
            ...
            if (in_array($row->sub_type, ['fund_transfer', 'deposit']) && !empty($row->transfer_transaction)) {
                if($row->contact_name){
                    ...
                    $details .= '<br><b>' .$row->user_type. ':</b> ' . $row->contact_name . '<br>';
                }
                if ($row->type == 'credit') {
