        ...
        if ($request->ajax()) {
            ...
            if ($displaytype == '2') {
                foreach ($dates as $key => $day) {
                    /*Expanse Calcuation */
                    $getExpanses = Transaction::select('transactions.expense_category_id', DB::raw('IFNULL(sum(final_total),0) as total_expense'))->leftjoin('expense_categories AS ec', 'transactions.expense_category_id', '=', 'ec.id')->where('transactions.business_id', $business_id)->where('type', 'expense')
                    ...
                    $getSalesTax = Transaction::where('transactions.business_id', $business_id)->where('type', 'sell')->whereDate('transaction_date', $day);
                    if ($location_id) {
                        $getSalesTax = $getSalesTax->where('location_id', $location_id);
                    }
                    $getSalesTax = $getSalesTax->sum('tax_amount');

                    $getSales = Transaction::where('transactions.business_id', $business_id)->where('type', 'sell')->whereDate('transaction_date', $day);
                    if ($location_id) {
                        $getSales = $getSales->where('location_id', $location_id);
                    }
                    $getSales = $getSales->sum('final_total');

                    $getDueSales = Transaction::where('transactions.business_id', $business_id)->where('type', 'sell')->whereDate('transaction_date', $day)->where('payment_status', 'due');
                    if ($location_id) {
                        $getDueSales = $getDueSales->where('location_id', $location_id);
                    }
                    $getDueSales = $getDueSales->sum('final_total');
                    ...
                    /*Tax Calcuation */
                    $getTax = Transaction::where('transactions.business_id', $business_id)->where('type', 'sell')->whereDate('transaction_date', $day);
                    if ($location_id) {
                        $getTax = $getTax->where('location_id', $location_id);
                    }
                    $getTax = $getTax->sum('tax_amount');

                    $taxReport[] = $getTax;

                    /*Cost Of Goods */
                    $salesProductIds = null;
                    $countProduct = 0;
                    $costOfGoodsTotal = 0;
                    $salesProductIds = Transaction::join('transaction_sell_lines as pl', 'transactions.id', '=', 'pl.transaction_id')->where('transactions.business_id', $business_id)->whereDate('transaction_date', $day)->where('transactions.type', 'sell');
                    if ($location_id) {
                        $salesProductIds = $salesProductIds->where('location_id', $location_id);
                    }
                    $salesProductIds = $salesProductIds->select('pl.product_id as product_id', 'transactions.id as transactions_id', 'pl.quantity as quantity')->get()->toArray();
                    $totalProductAmount = 0;
                    foreach ($salesProductIds as $spid) {
                        $productDefaultPrice = Variation::where('product_id', $spid['product_id'])->sum('default_purchase_price');
                        $totalProductAmount += ($productDefaultPrice * $spid['quantity']);
                    }

                    $countProduct = count($salesProductIds);

                    if ($totalProductAmount) {
                        $costOfGoodsTotal = $totalProductAmount;
                    }
                ...
                foreach ($monthArray as $key => $month) {
                    ...
                    $getSalesTax = Transaction::where('transactions.business_id', $business_id)->where('type', 'sell')->whereYear('transaction_date', '=', $year[$key])->whereMonth('transaction_date', '=', $month);
                    if ($location_id) {
                        $getSalesTax = $getSalesTax->where('location_id', $location_id);
                    }
                    $getSalesTax = $getSalesTax->sum('tax_amount');

                    $getSales = Transaction::where('transactions.business_id', $business_id)->where('type', 'sell')->whereYear('transaction_date', '=', $year[$key])->whereMonth('transaction_date', '=', $month);
                    if ($location_id) {
                        $getSales = $getSales->where('location_id', $location_id);
                    }
                    $getSales = $getSales->sum('final_total');

                    $getDueSales = Transaction::where('transactions.business_id', $business_id)->where('type', 'sell')->whereYear('transaction_date', '=', $year[$key])->whereMonth('transaction_date', '=', $month)->where('payment_status', 'due');
                    if ($location_id) {
                        $getDueSales = $getDueSales->where('location_id', $location_id);
                    }
                    $getDueSales = $getDueSales->sum('final_total');
                    ...
                    $shippingReport[] = $getShipping;
                    /*Tax Calcuation */
                    $getTax = Transaction::where('transactions.business_id', $business_id)->where('type', 'sell')->whereYear('transaction_date', '=', $year[$key])->whereMonth('transaction_date', '=', $month);
                    if ($location_id) {
                        $getTax = $getTax->where('location_id', $location_id);
                    }
                    $getTax = $getTax->sum('tax_amount');
                    ...
                    $taxReport[] = $getTax;
                    //$taxReport[] = $getSales*0.16;

                    /*Cost Of Goods */
                    $salesProductIds = null;
                    $countProduct = 0;
                    $costOfGoodsTotal = 0;
                    $salesProductIds = Transaction::join('transaction_sell_lines as pl', 'transactions.id', '=', 'pl.transaction_id')->where('transactions.business_id', $business_id)->whereYear('transaction_date', '=', $year[$key])->whereMonth('transaction_date', '=', $month)->where('transactions.type', 'sell');
                    if ($location_id) {
                        $salesProductIds = $salesProductIds->where('location_id', $location_id);
                    }
                    $salesProductIds = $salesProductIds->select('pl.product_id as product_id', 'transactions.id as transactions_id', 'pl.quantity as quantity')->get()->toArray();
