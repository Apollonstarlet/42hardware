...
class TransactionUtil extends Util
{
    ...
    protected function _receiptDetailsSellLines($lines, $il, $business_details)
    {
        ...
        foreach ($lines as $line) {
            ...
            $item_tax_rate = TaxRate::where('id', $line->product->tax)->first();
             //var_dump($item_tax_rate->amount);exit;
            ...
            $line_array = [
                ...
                'item_tax_rate'=>$item_tax_rate->amount,
                'product_unit_price_inc_tax' => $this->num_f(round($line->unit_price + (($line->unit_price*$item_tax_rate->amount)/100))),
                //Fields for 4th column
                'line_total' => $this->num_f(round($line->unit_price_inc_tax * $line->quantity + ($line->unit_price_inc_tax * $line->quantity)* $item_tax_rate->amount/100) , false, $business_details),
            ];
