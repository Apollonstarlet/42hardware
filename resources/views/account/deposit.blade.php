    ...
    <div class="modal-body">
      ...
      <div class="form-group">
        <label for="supplier_id">User Type:</label>
          <select class="form-control select2" id="deposit_user_type" name="user_type">
            <option selected="selected" value="">Select User Type</option>
            <option value="customer">Customer</option>
            <option value="supplier">Supplier</option>
          </select>
      </div>
      <div class="form-group deposit_supplier_dd" style="display: none;">
        {!! Form::label('deposit_supplier_id', __('purchase.supplier') . ':') !!}
        <div class="input-group">
          <span class="input-group-addon">
            <i class="fa fa-user"></i>
          </span>
          {!! Form::select('deposit_supplier_id', $suppliers, null, ['class' => 'form-control select2', 'placeholder' => __('lang_v1.all')]); !!}
        </div>
      </div>
      <div class="form-group deposit_customer_dd" style="display: none;">
        {!! Form::label('deposit_customer_id', __('Customer') . ':') !!}
        <div class="input-group">
          <span class="input-group-addon">
            <i class="fa fa-user"></i>
          </span>
          {!! Form::select('deposit_customer_id', $customers, null, ['class' => 'form-control select2', 'placeholder' => __('lang_v1.all')]); !!}
        </div>
      </div>
      <div class="form-group">
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    $("#deposit_supplier_id").select2({});
    $("#deposit_customer_id").select2({});
  });
</script>
<style>
  .deposit_customer_dd .select2-container, .deposit_supplier_dd .select2-container{
    width: 100% !important;
  }
</style>